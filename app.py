...
""")
                    ...
                    for mf_ in mf_list:
                        for ms_ in ms_list:
                            for s_ in sig_list:
                                ...
                                df_ci, md_i_, sh_i_, wr_i_ = backtest(build_composite(df_i, mw, rp))
                                strat_return = (df_ci["CumStrat"].iloc[-1] - 1)*100
                                results.append({
                                    "MA": mw, "RSI": rp, "MACD_Fast": mf_, "MACD_Slow": ms_, "MACD_Sig": s_,
                                    "Strategy %": strat_return,
                                    "Sharpe": sh_i_, "Max Drawdown %": md_i_, "Win Rate %": wr_i_
                                })
        if results:
    ...
    if st.button("📬 Generate Watchlist Summary"):
        ...
        for t in df_watch.index:
            ...
            if df_t.empty:
                continue
            df_c = build_composite(df_t, ma_window, rsi_p)
            last = df_c.iloc[-1]
