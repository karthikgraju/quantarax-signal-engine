...
with tab_engine:
    ...
    if st.button("▶️ Run Composite Backtest"):
        df_raw = load_and_compute(ticker,
                                  ma_window, rsi_period,
                                  macd_fast, macd_slow, macd_signal)
        if df_raw.empty:
            st.error(f"No data for '{ticker}'")
            st.stop()

        df_c, max_dd, sharpe, win_rt = backtest(
            build_composite(df_raw, ma_window, rsi_period)
        )
        ...
        ma_txt  = {1:f"Price crossed **above** its {ma_window}-day MA.",
                   0:"No MA crossover.",
                  -1:f"Price crossed **below** its {ma_window}-day MA."}.get(ma_s)
        rsi_txt = {1:f"RSI ({rsi_v:.1f}) < 30 → oversold.",
                   0:f"RSI ({rsi_v:.1f}) between 30–70 → neutral.",
                  -1:f"RSI ({rsi_v:.1f}) > 70 → overbought."}.get(rsi_s)
        macd_txt= {1:"MACD line crossed **above** signal.",
                  0:"No MACD crossover.",
                 -1:"MACD line crossed **below** signal."}.get(macd_s)
...
""")
        ...
        axs[1].bar(df_c.index, df_c["Composite"], color="purple")
    ...
    batch = st.text_area("Tickers (comma-separated)",
                         "AAPL, MSFT, TSLA, SPY, QQQ").upper()
    if st.button("▶️ Run Batch Backtest"):
        perf=[]
        for t in [x.strip() for x in batch.split(",") if x.strip()]:
            df_t = load_and_compute(t,
                                    ma_window, rsi_period,
                                    macd_fast, macd_slow, macd_signal)
            if df_t.empty: continue
            df_t_c, md, sh, wr = backtest(
                build_composite(df_t, ma_window, rsi_period)
            )
            perf.append({
                "Ticker":t,
                "Composite":int(df_t_c["Composite"].iloc[-1]),
                "Signal":{1:"BUY",0:"HOLD",-1:"SELL"}[
                    int(df_t_c["Trade"].iloc[-1])
                ],
                "BuyHold %":(df_t_c["CumBH"].iloc[-1]-1)*100,
                "Strategy %":(df_t_c["CumStrat"].iloc[-1]-1)*100,
                "Sharpe":sh,
                "MaxDD %":md,
                "Win Rate":wr
            })
    ...
    ma_list  = st.sidebar.multiselect("MA windows",[5,10,15,20,30],
                                      default=[ma_window], key="grid_ma")
    rsi_list = st.sidebar.multiselect("RSI lookbacks",[7,14,21,28],
                                      default=[rsi_period], key="grid_rsi")
    mf_list  = st.sidebar.multiselect("MACD fast spans",[8,12,16,20],
                                      default=[macd_fast],   key="grid_mf")
    ms_list  = st.sidebar.multiselect("MACD slow spans",[20,26,32,40],
                                      default=[macd_slow],   key="grid_ms")
    sig_list = st.sidebar.multiselect("MACD sig spans",[5,9,12,16],
                                      default=[macd_signal],key="grid_sig")

    if st.button("🏃‍♂️ Run Grid Search"):
        if not ticker:
            st.error("Enter a ticker."); st.stop()
        df_f = load_and_compute(ticker,
                                ma_window, rsi_period,
                                macd_fast, macd_slow, macd_signal)
        if df_f.empty:
                    ...
                    for mf in mf_list:
                        for ms in ms_list:
                            for s in sig_list:
                                ...
                                df_c_i, md_i, sh_i, wr_i = backtest(
                                    build_composite(df_i, mw, rp)
                                )
                                strat = (df_c_i["CumStrat"].iloc[-1]-1)*100
                                results.append({
                                    "MA":mw, "RSI":rp,
    ...
    watch = st.text_area("Enter tickers (comma-separated)",
                         "AAPL, MSFT, TSLA, SPY, QQQ").upper()
    if st.button("📬 Generate Watchlist Summary"):
        ...
        for t in [x.strip() for x in watch.split(",") if x.strip()]:
            ...
            if df_t.empty:
                ...
                continue
            df_w,_,_,_= backtest(build_composite(df_t,
                                                ma_window,
                                                rsi_period))
        ...
        for t in df_watch.index:
            ...
            last = df_c.iloc[-1]
            ma_s, rsi_s, macd_s = (int(last[c]) for c in
                ["MA_Signal","RSI_Signal","MACD_Signal2"])
            ...
            macd_txt = {
                1:"MACD crossed **above** signal line.",
                0:"No MACD crossover.",
               -1:"MACD crossed **below** signal line."
            }[macd_s]
