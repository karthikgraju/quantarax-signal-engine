...
with tab_engine:
    ...
    def load_and_compute(ticker, ma_w, rsi_p, mf, ms, sig):
        ...
        # Drop NA on new cols, safely
        to_drop = [c for c in (ma_col, rsi_col, "MACD", "MACD_Signal") if c in df.columns]
        if to_drop:
            try:
...
""")
    ...
    watch = st.text_area("Enter your watchlist tickers", "AAPL, MSFT, TSLA, SPY, QQQ").upper()
