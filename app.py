...
with tab_engine:
    ...
    if st.button("▶️ Run Composite Backtest"):
        ...
        # Why this signal? (reuse your reasoning expander code here)
        # ...

        # ─────────── Plotly Candlestick + Signals ───────────
        fig = go.Figure(data=[
            go.Candlestick(
                x=df_c.index,
                open=df_c["Open"],
            ...
            go.Scatter(
                x=df_c.index[df_c["Trade"]==1],
                y=df_c["Low"][df_c["Trade"]==1] * 0.995,
                mode="markers",
            ...
            go.Scatter(
                x=df_c.index[df_c["Trade"]==-1],
                y=df_c["High"][df_c["Trade"]==-1] * 1.005,
                mode="markers",
                        ...
                        for ms_ in ms_list:
                            for s_ in sig_list:
                                ...
                                results.append({
                                    ...
                                    "Strategy %":strat, "Sharpe":sh_i, "MaxDD %":md_i, "Win %":wr_i
                                })
        if results:
