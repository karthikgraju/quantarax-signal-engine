# app.py — QuantaraX v5 Pro (Part 1/3)
# ---------------------------------------------------------------------------------
# New in v5:
# - Adaptive thresholds & position sizing
# - Meta-ensemble (learned composite) scaffolding
# - Event engine (earnings/FOMC) scaffolding
# - Options lens (ATM IV + skew + IV-rank on chain snapshot)
# - Robust backtester (costs, ATR exits, vol targeting) retained
#
# Paste Part 1, then wait for Part 2/3 (UI + Engine + Scanner + ML purged-kfold)
# and Part 3/3 (Regimes HMM, Black-Litterman, Strategy Studio, Alerts, Help).
...
# ───────────────────────────── Page Setup ─────────────────────────────
st.set_page_config(page_title="QuantaraX v5 Pro", layout="wide")
...
def _zscore(x: pd.Series, win: int = 20) -> pd.Series:
    mu = x.rolling(win).mean()
    sd = x.rolling(win).std(ddof=0).replace(0, np.nan)
    return (x - mu) / sd

def _smooth(series: pd.Series, alpha: float = 0.2) -> pd.Series:
...
def build_composite(df: pd.DataFrame, ma_w: int, rsi_p: int,
    ...
    """Adds adaptive thresholds + position sizing, returns df with Trade, Size, ThrAdj."""
    if df.empty:
        return df.copy()
    ...
    # --- Adaptive threshold & position sizing ---
    comp_raw = pd.Series(comp, index=d.index, dtype="float64")
    atr_norm = (d["ATR"] / d["Close"]).clip(lower=1e-8)
    vol_ratio = (atr_norm / atr_norm.rolling(20).mean()).clip(lower=0.5, upper=2.0)
    thr_adj = max(0.1, float(threshold)) * vol_ratio
    thr_adj = thr_adj.fillna(float(threshold))

    if allow_short:
        trade_adapt = np.where(comp_raw >= thr_adj, 1, np.where(comp_raw <= -thr_adj, -1, 0))
    else:
        trade_adapt = np.where(comp_raw >= thr_adj, 1, 0)

    size = (comp_raw.abs() / (thr_adj * 2.0)).clip(0.0, 1.0)
    size = _smooth(size, alpha=0.25).fillna(0.0)

    d["MA_Signal"], d["RSI_Signal"], d["MACD_Signal2"] = ma_sig, rsi_sig, macd_sig2
    if include_bb:
        d["BB_Signal"] = bb_sig
    d["Composite"] = comp_raw.astype(float)
    d["Trade"] = pd.Series(trade_adapt, index=d.index).astype(int)
...
def backtest(df: pd.DataFrame, *, allow_short=False, cost_bps=0.0,
    ...
    # Base position uses Trade * Size (adaptive sizing)
    sig = d.get("Trade", 0).shift(1).fillna(0)
    siz = d.get("Size", 1.0).shift(1).fillna(1.0).clip(0.0, 1.5)

    if allow_short:
        d["Position"] = (sig * siz).clip(-1.5, 1.5)
        base_ret = np.where(d["Position"] >= 0, d["Return"], -d["Return"]) * d["Position"].abs()
    else:
        d["Position"] = (sig.clip(lower=0) * siz).clip(0.0, 1.5)
...
def get_earnings_dates(symbol: str, limit: int = 12) -> List[dt.date]:
    """Try multiple yfinance endpoints for earnings dates."""
    dates: List[dt.date] = []
    try:
        t = yf.Ticker(_map_symbol(symbol))
        # Preferred: get_earnings_dates
        try:
            ed = t.get_earnings_dates(limit=limit)
            if isinstance(ed, pd.DataFrame) and not ed.empty:
                dates = pd.to_datetime(ed.index).date.tolist()
        except Exception:
            pass
        # Fallback: calendar
        if not dates:
            ...
            if isinstance(cal, pd.DataFrame) and not cal.empty:
                # Commonly 'Earnings Date' in index or columns
                if "Earnings Date" in cal.index:
                    raw = cal.loc["Earnings Date"].values
                    for v in _safe_list(raw):
                        try:
                    ...
                    for v in cal["Earnings Date"]:
                        try:
                            ...
                            pass
        # Dedup & sort
        dates = sorted(list({d for d in dates if isinstance(d, dt.date)}))
    except Exception:
        dates = []
    return dates[:limit]

# Rough FOMC schedule (sample; users can add more via UI)
FOMC_DATES = [
    dt.date(2025, 1, 29), dt.date(2025, 3, 19), dt.date(2025, 5, 7),
    dt.date(2025, 6, 18), dt.date(2025, 7, 30), dt.date(2025, 9, 17),
    dt.date(2025,11, 5), dt.date(2025,12, 17)
]

def apply_event_flatten(df: pd.DataFrame, event_dates: List[dt.date], days_before: int = 1, days_after: int = 0) -> pd.DataFrame:
    """Flattens Trade around events; leaves Composite for visibility."""
    ...
    for ed in event_dates:
        if not isinstance(ed, dt.date): 
            continue
        lo = ed - dt.timedelta(days=days_before)
        hi = ed + dt.timedelta(days=days_after)
        mask |= (idx_dates >= lo) & (idx_dates <= hi)
    if mask.any():
        d.loc[mask, "Trade"] = 0
        d["EventMask"] = mask.astype(int)
    else:
        d["EventMask"] = 0
    return d
...
def options_lens(symbol: str) -> Optional[Dict[str, float]]:
    """Snapshot lens: ATM IV, simple IV-rank across current chain, smile skew."""
    try:
        sym = _map_symbol(symbol)
        t = yf.Ticker(sym)
        # spot
        ...
        try:
            ...
            spot = finfo.get("last_price", None)
        except Exception:
            pass
        if spot is None:
        ...
        exps = _safe_list(getattr(t, "options", []))
        if not exps:
            return None
        exp = exps[0]
        ...
        # ATM IV: pick nearest strike to spot across both
        atm_row = pd.concat([calls, puts], axis=0).iloc[(pd.concat([calls, puts])["strike"] - spot).abs().argmin()]
        iv_atm = float(atm_row["impliedVolatility"])

        # Skew: put IV at ~0.9 spot minus call IV at ~1.1 spot
        def _closest_iv(df: pd.DataFrame, target_mny: float) -> float:
            row = df.iloc[(df["moneyness"] - target_mny).abs().argmin()]
            return float(row["impliedVolatility"])
        put_iv  = _closest_iv(puts, 0.90)
        call_iv = _closest_iv(calls, 1.10)
        skew = put_iv - call_iv

        # IV rank within current chain snapshot (not historical!)
        all_iv = pd.concat([calls["impliedVolatility"], puts["impliedVolatility"]], axis=0).dropna()
        iv_min, iv_max = float(all_iv.min()), float(all_iv.max())
        iv_rank = (iv_atm - iv_min) / (iv_max - iv_min) if iv_max > iv_min else 0.5

        return {"spot": spot, "iv_atm": iv_atm, "skew": skew, "iv_rank_chain": iv_rank}
    except Exception:
        return None
# ───────────────────────────── Sidebar & Tabs ─────────────────────────────
...
comp_thr = st.sidebar.slider("Composite trigger (base)", 0.0, 3.0, 1.0, 0.1)
...
vol_target  = st.sidebar.slider("Vol targeting (annual)", 0.0, 0.5, 0.0, 0.05)
...
guard_earn  = st.sidebar.toggle("Flatten around earnings", value=True)
earn_bef    = st.sidebar.slider("Days before earnings (flat)", 0, 5, 1)
earn_aft    = st.sidebar.slider("Days after earnings (flat)",  0, 5, 0)
guard_fomc  = st.sidebar.toggle("Flatten around FOMC (sample dates)", value=False)

# Options lens
st.sidebar.subheader("Options Lens")
show_options = st.sidebar.toggle("Show ATM IV / Skew / IV-rank", value=False)

# Portfolio guardrails (for simulator later)
...
# ───────────────────────────── ENGINE ─────────────────────────────
with tab_engine:
    st.title("🚀 QuantaraX v5 — Composite Signal Engine")

    # Live price (non-blocking)
    ticker = st.text_input("Ticker (e.g. AAPL or BTC/USDT)", "AAPL").upper().strip()
    if ticker:
        try:
            h = yf.download(_map_symbol(ticker), period="1d", progress=False)
            if not h.empty and "Close" in h:
                price = float(h["Close"].iloc[-1].item() if hasattr(h["Close"].iloc[-1], "item") else h["Close"].iloc[-1])
                st.subheader(f"💲 Live Price: ${price:,.2f}")
        except Exception:
            pass

        # News via RSS only (avoid yfinance .news failures)
    ...
    run_btn = st.button("▶️ Run Composite Backtest")
...
# ───────────────────────────── ML LAB (Purged K-Fold) ─────────────────────────────
with tab_ml:
    st.title("🧠 ML Lab — Purged K-Fold, OOS Probabilities")
    if not SKLEARN_OK:
        st.warning("scikit-learn not installed. Run: pip install scikit-learn")

    symbol = st.text_input("Symbol (ML)", value="AAPL").upper().strip()
    horizon = st.slider("Prediction horizon (bars)", 1, 5, 1)
    n_splits = st.slider("TimeSeries splits", 3, 8, 5)
    gap = st.slider("Purging gap (bars)", 0, 20, 5)
    proba_enter = st.slider("Enter if P(long) ≥", 0.50, 0.90, 0.60, 0.01)
    proba_exit  = st.slider("Enter short if P(long) ≤", 0.10, 0.50, 0.40, 0.01)
    run_ml = st.button("🤖 Train & Backtest (Purged CV)")
    ...
    if run_ml and SKLEARN_OK:
        try:
            ...
            # keep adaptive size 1.0 for ML, or scale by confidence:
    ...
    if run_scan:
        ...
        for t in tickers:
            try:
                ...
                # Apply event guards for uniformity
                if guard_earn:
        ...
        if rows:
            ...
            st.download_button("Download CSV", df.to_csv(), "scan.csv")
        else:
            st.info("No results. Check tickers or increase history.")
# app.py — QuantaraX Pro (v5)
# ---------------------------------------------------------------------------------
...
# ───────────────────────────── Page Setup ─────────────────────────────
st.set_page_config(page_title="QuantaraX Pro v5", layout="wide")
analyzer = SentimentIntensityAnalyzer()

rec_map = {1: "🟢 BUY", 0: "🟡 HOLD", -1: "🔴 SELL"}

# Sample past FOMC dates for shading/guarding (extend as you like)
FOMC_DATES = [
...
def build_composite(df: pd.DataFrame, ma_w: int, rsi_p: int,
    ...
    # Adaptive threshold & size: vol-aware
...
def apply_event_flatten(df: pd.DataFrame, dates: List[dt.date],
    ...
    d["EventMask"] = mask
    # Flatten signal (zero) where mask == 1
    if "Trade" in d.columns:
        d.loc[d["EventMask"]==1, "Trade"] = 0
    return d

def options_lens(symbol: str) -> Optional[Dict[str, float]]:
    """Quick ATM IV, skew and within-chain IV-rank snapshot. Returns None if not available."""
    try:
        tk = yf.Ticker(_map_symbol(symbol))
        exps = tk.options
        if not exps:
            return None
        # pick nearest expiry with a chain
        exp = exps[0]
        ...
        # ATM IV: average of call/put IV around ATM
        ...
        # Skew: 90% put IV - 110% call IV (as a difference)
        k_put = 0.90*spot
        k_call = 1.10*spot
        ...
        # IV rank within this single chain (not multi-year historical IV rank)
...
# ───────────────────────────── ENGINE ─────────────────────────────
with tab_engine:
    st.title("🚀 QuantaraX — Composite Signal Engine (v5)")

    # Live price + RSS news (avoid yfinance .news)
    ticker = st.text_input("Ticker (e.g. AAPL or BTC/USDT)", "AAPL").upper().strip()
    if ticker:
        try:
            h = yf.download(_map_symbol(ticker), period="1d", progress=False)
    ...
    if run_rg:
        try:
            ...
            if HMM_OK:
                ...
                # Quantile fallback: 3 bins on mean rank
            ...
            # Reorder regimes by mean forward return
            fwd = joined["Close"].pct_change().shift(-5)  # 5d fwd
...
with tab_port:
    ...
    if st.button("🧠 Compute BL Weights"):
        try:
            ...
            R.columns = ticks = [t for t in tickers if t in R.columns or True][:len(R.columns)]

            mu = R.mean()*252
            Sigma = R.cov()*252
            n = len(Sigma)
            w_mkt = np.ones(n)/n  # unknown market caps → equal weight
            delta = 3.0
            tau = 0.05
            ...
            if not P:
                st.info("No valid views parsed. Using equilibrium (== mean-variance).")
    ...
    # ---------- Strategy Studio (simple rule builder) ----------
    st.subheader("🧪 Strategy Studio (sandbox)")
    ss_symbol = st.text_input("Symbol (Studio)", value="AAPL").upper()
    ss_kind = st.selectbox("Strategy type", ["MA Trend", "Donchian Breakout", "RSI Mean-Reversion"])
    ss_len1 = st.number_input("Len1 (fast/RSI/DC)", 5, 200, 10, 1)
    ss_len2 = st.number_input("Len2 (slow)", 5, 400, 50, 1)
    if st.button("🏃 Run Studio Backtest"):
        try:
...
# ───────────────────────────── HELP ─────────────────────────────
with tab_help:
    st.header("How QuantaraX Pro v5 Works")
    st.markdown(r"""
**QuantaraX Pro v5** packs:
...
- **Robust Backtester**: long/short, trading costs, ATR stops/targets, volatility targeting.
- **Options Lens**: ATM IV, skew, within-chain IV-rank.
- **ML Lab**: Purged TimeSeries CV, OOF probabilities → strategy backtest & feature importances.
- **Regimes**: Hidden Markov Model (if `hmmlearn` installed) or KMeans fallback.
- **Portfolio**: Risk Parity, **Black–Litterman** with simple views, **Strategy Studio**, Monte Carlo.

> Tips:
> - If you see timeouts from Yahoo, just rerun—free endpoints can throttle.
> - For HMM regimes: `pip install hmmlearn`.
> - For best ML stability, use **daily** interval and **2–5 years** of data.
""")
